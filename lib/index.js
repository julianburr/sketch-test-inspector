...
function runPluginCommand (identifier, options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.writeJsonSync(ACTIONS_FILE_PATH, []);

    // We write sketch actions to a specitic file, so we can listen to this file
    // to know in node when an action has been fired
    watch.createMonitor(`${INSPECTOR_PLUGIN_PATH}/Contents/Resources/`, monitor => {
      monitor.on('changed', (f, curr, prev) => {
        if (f === ACTIONS_FILE_PATH) {
          // RunPluginCommand.finish
          monitor.stop();
          saveDocument();
          // Timeout to make sure document is saved before resolving
          // So we for sure get the fresh data
